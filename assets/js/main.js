...
var proccessMsgsArr = function (msgs) {
	...
	for (var $thisMsg in $msgs) {
		...
		$elm += $msgId;
		$elm += '</span>';
		$elm += '<button class="delete-msg-btn" data-message-id="' + $msgId + '">Delete</button>';
...
var sendImageMsg = function (file) {
	$(".send_msg_form").addClass("disabled");

	if ($.globals.isPendingMsg) {
		console.error("sendImageMsg called while another process is running");
		$(".send_msg_form").removeClass("disabled");
		return false;
	}
	...
	if (!$username || !$contactId) {
		console.error("Missing username or contactId");
		$(".send_msg_form").removeClass("disabled");
		$.globals.isPendingMsg = 0;
		return false;
	}
	...
	// Use your existing server endpoint for uploading images (you may need to create or adapt one)
	$.ajax({
		url: "upload_images.php",  // Adjust to your image upload PHP endpoint
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
		success: function (response) {
			if (response.success) {
				console.log("Image uploaded:", response.file);

				// Now send message with image path in msg_body and msg_type = 'image'
				var postData = {
					msg: response.file, // or full URL if your server returns full path
					username: $username,
					contact_id: $contactId,
					msg_type: "image",
				};
				...
				postToServer({
					data: postData,
					route: "send_wa_txt_msg", // Or use a separate route if you want for images
					successCallback: function (data) {
						$(".send_msg_form").removeClass("disabled");
						$.globals.isPendingMsg = 0;
						resetAllForms();
...
$(document).ready(function () {
	...
	// Open file dialog on paperclip click
	...
	// Drag & drop support on chat window
	...
	$('#chat_window').on('dragleave drop', function (e) {
		e.preventDefault();
		e.stopPropagation();
	...
	function showImagePreview(file) {
		...
		reader.onload = function (e) {
			$('#image_preview_container').html(`<img src="${e.target.result}" alt="Selected Image Preview" />`);

			// Optional: clear text input or disable to avoid conflict
			$('#msg').val('');
		};
...
$(window).on("load", function () {
	...
	$("body").on("click", ".delete-msg-btn", async function () {
		e.preventDefault();

		const messageId = $(this).data('message-id');
		$.post('delete_message.php', { message_id: messageId }, function (response) {
	...
	$("body").on("submit", "#send_msg", function (e) {
		...
		var fileInput = $("#image_input")[0]; // Your file input element, you may need to add this if not present
		console.log('✌️$("#image_input") --->', $("#image_input"));
		console.log('✌️fileInput --->', fileInput);
		var hasFile = fileInput && fileInput.files.length > 0;
		if (!$(this).hasClass("disabled")) {
			if (hasFile) {
				sendImageMsg(fileInput.files[0]);
			} else {
				sendTxtMsg($msg);
			}
		}
	...
	emojiBtn.addEventListener('click', () => {
		if (emojiPicker.style.display === 'none' || !emojiPicker.style.display) {
			...
			emojiPicker.style.display = 'unset'; // Use content-box to avoid overflow issues
		} else {
			emojiPicker.style.display = 'none';
		}
	});
	//  style="position: absolute; display:none;   right: 0;    bottom: 80%;"></emoji-picker>


	// When an emoji is selected, add it to the input at the cursor position
